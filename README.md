...
- Активность пользователей: 
    1. В среднем пользователь просматривает просматривает ленту (постов + мест + комментарии) ~ 100 запросов в день
    2. В среднем пользователь просматривает просматривает ленту (фото) 100 * 10 = 500 запросов в день
...
API JSON: ~8–10 KB gzip/ответ (20 постов). Возьмём 9 KB.
11,574 rps × 9 KB ≈ ~104,166 KB/s ≈ 101.8 MB/s ≈ 815 Mb/s.

Картинки (thumbnails): возьмём 100 KB средний превью.
34,722 rps × 100 KB ≈ 3,472,200 KB/s ≈ 3,392 MB/s ≈ 27.1 Gb/s (среднее).
Пик ×3 → ~80 Gb/s (раздаёт CDN, не ваш origin).